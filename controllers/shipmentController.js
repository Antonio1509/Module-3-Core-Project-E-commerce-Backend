...
export const getVendorStats = async (req, res) => {
  try {
    ...
    const stats = rows[0] || { total: 0, collected: 0, awaiting: 0, returns: 0 };
    res.json(stats);
  } catch (error) {
...
export const getUnshippedOrders = async (req, res) => {
  try {
    ...
    const orders = rows.map(r => ({
      ...
      items: (itemsByOrder[r.orderId] || []).map(i => [i.name, i.qty, i.price]),
    }));
...
export const createShipment = async (req, res) => {
  try {
    ...
    if (method === 'rankdrop' && !pickupPoint) {
      return res.status(400).json({ message: 'pickupPoint is required for RankDrop' });
    }


    // Verify order belongs to vendor and isn't already shipped
    const [orderRows] = await pool.query(
      `SELECT o.id, o.order_number, u.name AS customer, o.shipping_address
       FROM orders o
       JOIN order_items oi ON oi.order_id = o.id
       JOIN users u        ON u.id = o.user_id
       LEFT JOIN shipments s ON s.order_id = o.id AND s.vendor_id = ?
    ...
    const finalPickupPoint =
      method === 'rankdrop' ? pickupPoint : 'Vendor self-delivery';
