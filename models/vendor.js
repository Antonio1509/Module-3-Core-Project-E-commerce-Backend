/* =========================================================
   LocalCart — models/vendor.js
   ES6 class wrapping every vendor-related SQL query in one
   place. vendorController.js can call these instead of writing
   raw SQL inline — same queries, same schema, just organized
   as a data-access layer.

   Methods that need to participate in a transaction (create)
   accept an optional `db` parameter — pass a transaction
   connection from pool.getConnection() when you need atomicity
   across multiple inserts; otherwise the shared pool is used.
   ========================================================= */
...
class Vendor {
  ...
  /**
   * A single vendor's public profile. Never includes the
   * linked user's email/phone — see getContactInfo() for that.
   */
